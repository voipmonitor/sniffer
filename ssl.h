...
#define SE_COPY_ADDRESS(to, from)	 \
	do {							  \
		...
		SE_COPY_ADDRESS_data = new FILE_LINE(33001) guchar[(from)->len]; \
...
static gboolean from_hex(StringInfo* out, const char* in, gsize hex_len) {
	...
	out->data = new FILE_LINE(33002) guchar[hex_len / 2];
...
bytes_to_ep_str(const guint8 *bd, int bd_len)
{
		...
		cur = new FILE_LINE(33003) gchar[MAX_BYTE_STR_LEN+3+1];
...
{	  
		...
		cur = new FILE_LINE(33004) gchar[MAX_BYTE_STR_LEN+3+1];
		if (bd_len <= 0) { cur[0] = '\0'; return cur; }
...
{                       
	str->data = new FILE_LINE(33005) guchar[len];
