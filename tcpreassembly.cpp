	...
	for(i = startIndex; i < this->ok_packets.size(); i++) {
		...
		if(PACKET_DATALEN(packet.datalen, packet.datacaplen)) {
			...
			if(!data) {
				...
				data = new FILE_LINE(37001) u_char[databuff_len];
				...
				u_char* newdata = new FILE_LINE(37002) u_char[databuff_len];
...
bool TcpReassemblyStream::checkOkPost(TcpReassemblyStream *nextStream) {
	...
	char *data = new FILE_LINE(37003) char[datalen + 1];
	...
	for(int i = 0; i < 3; i++) {
		if(i == 0 ? datalen > 0 : 
		   i == 1 ? header_tcp.syn || header_tcp.fin || header_tcp.rst : 
			...
			if(iter == queue->end()) {
				stream = new FILE_LINE(37004) TcpReassemblyStream(this);
...
void TcpReassemblyLink::pushpacket(TcpReassemblyStream::eDirection direction,
	...
	if(iter == this->queue_by_ack.end() || !iter->second) {
		...
		stream = new FILE_LINE(37005) TcpReassemblyStream(this);
		stream->direction = direction;
		stream->ack = packet.header_tcp.ack_seq;
		if(!reassembly->simpleByAck) {
		...
		while(countIgnore + countData + countRequest + countResponse < this->ok_streams.size()) {
			...
			if(reassembly->enableIgnorePairReqResp ||
			   ...
			   (stream->direction == TcpReassemblyStream::DIRECTION_TO_SOURCE && countRequest)) {
				if(!reassemblyData) {
					reassemblyData = new FILE_LINE(37006) TcpReassemblyData;
				}
				if(reassembly->enableIgnorePairReqResp) {
					++countData;
				} else {
...
void TcpReassemblyLink::complete_simple_by_ack() {
	...
	TcpReassemblyData *reassemblyData = new FILE_LINE(37007) TcpReassemblyData;
...
void TcpReassemblyLink::complete_crazy(bool final, bool eraseCompletedStreams) {
	while(true) {
		...
		reassemblyData = new FILE_LINE(37008) TcpReassemblyData;
...
void TcpReassemblyLink::createEthHeader(u_char *packet, int dlt) {
	...
	if(parseEtherHeader(dlt, packet,
		...
		if(this->ethHeaderLength > 0 && this->ethHeaderLength < 50) {
			this->ethHeader = new FILE_LINE(37009) u_char[this->ethHeaderLength];
...
void TcpReassemblyLink::setRemainData(u_char *data, u_int32_t datalen, TcpReassemblyDataItem::eDirection direction) {
	...
	if(index >= 0) {
		...
		if(data && datalen) {
			this->remainData[index] = new FILE_LINE(37010) u_char[datalen];
...
void TcpReassembly::_push(pcap_pkthdr *header, iphdr2 *header_ip, u_char *packet,
		...
		if(!this->enableCrazySequence &&
		   header_tcp.syn && !header_tcp.ack) {
			if(this->check_port(header_tcp.dest, htonl(header_ip->daddr))) {
				...
				link = new FILE_LINE(37011) TcpReassemblyLink(this, header_ip->saddr, header_ip->daddr, header_tcp.source, header_tcp.dest,
								       packet, header_ip,
								       handle_index, dlt, sensor_id, sensor_ip,
								       uData);
				this->links[id] = link;
			...
			if(type != ssl || 
			   this->check_port(header_tcp.dest, htonl(header_ip->daddr))) {
				...
				link = new FILE_LINE(37012) TcpReassemblyLink(this, header_ip->saddr, header_ip->daddr, header_tcp.source, header_tcp.dest,
								       packet, header_ip,
								       handle_index, dlt, sensor_id, sensor_ip,
								       uData);
				this->links[id] = link;
			...
			link = new FILE_LINE(37013) TcpReassemblyLink(this, header_ip->saddr, header_ip->daddr, header_tcp.source, header_tcp.dest,
							       packet, header_ip,
							       handle_index, dlt, sensor_id, sensor_ip,
							       uData);
			this->links[id] = link;
	...
	if(ENABLE_DEBUG(type, _debug_packet)) {
		string _data;
		if(datalen) {
			char *__data = new FILE_LINE(37014) char[datalen + 1];
