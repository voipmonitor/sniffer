...
//	if ((myargc = CreateArgs(argv[0], aLine, myargv)) < 0) {
	...
	char *argv[])
{
	...
	printf("uprvena verze rrd:\nZ prikazoveho radku ignoruji %d parametry\n",argc);
	int i=0;
	while (argv[i]){
		printf("%d:%s\n",i,argv[i]);
		i++;
	...
	char *command = "create latency_db.rrd --step 60 DS:pl:GAUGE:120:0:100 DS:rtt:GAUGE:120:0:10000000 RRA:MAX:0.5:1:1500";
//	res = rrd_exec(command);
	res = rrd_exec("update latency_db.rrd --template pl:rtt N:0:15.734");
