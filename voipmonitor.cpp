...
string opt_whisper_rest_api_mode = "stereo";
...
bool opt_cdr_problems_by_ip = true;
...
bool opt_diameter_ignore_leading_plus;
bool opt_diameter_match_user_session_id;
bool opt_diameter_match_msisdn;
string opt_diameter_user_session_id_strip_regex;
string opt_diameter_callid_strip_regex;
...
bool opt_t2_boost_ht_hash_queue = false;
bool opt_t2_boost_ht_cleanup_calls = false;
bool opt_t2_boost_ht_cleanup_registers = false;
...
Calltable::sCleanupRegistersData cr_data;
pthread_t cleanup_registers_thread;
...
bool opt_keep_final_last_sip_response = true;
bool opt_enable_premature_response = false;
...
int opt_client_server_receiver_timeout_s = 0;
			...
			if(connectOk > 0) {
				...
				if(!opt_database_backup) {
					if(!(opt_disable_dbupgradecheck || is_read_from_file_simple() ||
						...
						sqlDb->checkSchema(connectId, false);
						if(is_read_from_file_simple()) {
							SqlDb_mysql *sqlDb_mysql = dynamic_cast<SqlDb_mysql*>(sqlDb);
							if(sqlDb_mysql) {
...
void *cleanup_registers(void *) {
	...
	while(!is_terminating()) {
		...
		case Calltable::_cr_na: {
			...
			break;
		case Calltable::_cr_start:
			cr_data.state = Calltable::_cr_begin;
			calltable->cleanup_registers__begin(&cr_data);
			cr_data.state = Calltable::_cr_begin_finish;
			break;
		case Calltable::_cr_load_all_registers_finish:
			__sync_synchronize();
			if(cr_data.allRegisters) {
				cr_data.state = Calltable::_cr_process_registers;
				calltable->cleanup_registers__process_registers(&cr_data);
				__sync_synchronize();
				cr_data.state = Calltable::_cr_process_registers_finish;
			} else {
				cr_data.state = Calltable::_cr_goto_end;
			}
			break;
		case Calltable::_cr_remove_registers_from_map_finish:
			cr_data.state = Calltable::_cr_close_registers;
			calltable->cleanup_registers__close_registers(&cr_data);
			cr_data.state = Calltable::_cr_close_registers_finish;
			break;
		case Calltable::_cr_goto_end:
		case Calltable::_cr_close_registers_finish:
			cr_data.state = Calltable::_cr_end;
			calltable->cleanup_registers__end(&cr_data);
			cr_data.state = Calltable::_cr_end_finish;
			break;
		case Calltable::_cr_end_finish:
			cr_data.state = Calltable::_cr_na;
			break;
		default:
			break;
		}
...
void store_crash_bt_to_db() {
	if(!opt_nocdr && file_exists(opt_crash_bt_filename)) {
		...
		if(header_ok && bt.size()) {
			...
			if(!tmpOut.empty()) {
				system((escapeShellPath(binaryNameWithPath) + " | grep version > " + escapeShellPath(tmpOut) + " 2>/dev/null").c_str());
				...
				if(version_ok) {
					system((string("which addr2line > ") + escapeShellPath(tmpOut) + " 2>/dev/null").c_str());
							...
							for(unsigned pass = 0; pass < 2 && !addr2line_ok; pass++) {
								...
								if(posAddr != string::npos) {
									...
									if(posAddrEnd != string::npos) {
										...
										system(("addr2line -e " + escapeShellPath(binaryNameWithPath) + " " + escapeShellArgument(addr) + " > " + escapeShellPath(tmpOut) + " 2>/dev/null").c_str());
					...
					system((string("which gdb > ") + escapeShellPath(tmpOut) + " 2>/dev/null").c_str());
					...
					if(file_get_rows(tmpOut, &gdb_check_rows)) {
						...
						for(unsigned i = 0; i < coredumps.size(); i++) {
							...
							system(string(
							       ...
							       "quit\n"
							       "' | gdb " + escapeShellPath(binaryNameWithPath) + " " + escapeShellPath(coredump) + " 2>&1 >/dev/null"
							       ).c_str());
...
*/
	...
	*/
	...
	if(!is_read_from_file() && opt_fork && reloadLoopCounter == 0 && rightPSversion && bashPresent) {
		cSystemdService::detect();
		if(enable_wdt) {
...
int main_init_read() {
	...
	if(opt_t2_boost_ht_cleanup_registers) {
		vm_pthread_create("cleanup_registers",
				  &cleanup_registers_thread, NULL, cleanup_registers, NULL, __FILE__, __LINE__);
	}
...
void main_term_read() {
	...
	if(cleanup_calls_thread) {
		pthread_join(cleanup_calls_thread, NULL);
		cleanup_calls_thread = 0;
	}
	if(cleanup_registers_thread) {
		pthread_join(cleanup_registers_thread, NULL);
		cleanup_registers_thread = 0;
	}
...
void cConfig::addConfigItems() {
		...
		subgroup("main");
			...
			addConfigItem((new FILE_LINE(42080) cConfigItem_yesno("utc", &opt_sql_time_utc))
				...
				expert();
					...
					addConfigItem(new FILE_LINE(0) cConfigItem_yesno("t2_boost_ht_hash_queue", &opt_t2_boost_ht_hash_queue));
					addConfigItem(new FILE_LINE(0) cConfigItem_yesno("t2_boost_ht_cleanup_calls", &opt_t2_boost_ht_cleanup_calls));
					addConfigItem(new FILE_LINE(0) cConfigItem_yesno("t2_boost_ht_cleanup_registers", &opt_t2_boost_ht_cleanup_registers));
	...
	group("data storing");
		...
		subgroup("AUDIO");
			...
			addConfigItem(new FILE_LINE(0) cConfigItem_yesno("liveaudio", &opt_liveaudio));
				...
				addConfigItem(new FILE_LINE(0) cConfigItem_string("whisper_rest_api_mode", &opt_whisper_rest_api_mode));
		...
		subgroup("main");
			...
			addConfigItem(new FILE_LINE(0) cConfigItem_yesno("sip-message", &opt_sip_message));
				...
				addConfigItem(new FILE_LINE(0) cConfigItem_yesno("enable_content_type_application_csta_xml", &opt_enable_content_type_application_csta_xml));
					...
					addConfigItem(new FILE_LINE(0) cConfigItem_yesno("keep_final_last_sip_response", &opt_keep_final_last_sip_response));
					addConfigItem(new FILE_LINE(0) cConfigItem_yesno("enable_premature_response", &opt_enable_premature_response));
		subgroup("REGISTER");
	...
	group("diameter");
			...
			addConfigItem(new FILE_LINE(0) cConfigItem_ports("diameter_udp_ports", diameter_udp_portmatrix));
				...
				addConfigItem(new FILE_LINE(0) cConfigItem_yesno("diameter_ignore_leading_plus", &opt_diameter_ignore_leading_plus));
				addConfigItem(new FILE_LINE(0) cConfigItem_yesno("diameter_match_user_session_id", &opt_diameter_match_user_session_id));
				addConfigItem(new FILE_LINE(0) cConfigItem_yesno("diameter_match_msisdn", &opt_diameter_match_msisdn));
				addConfigItem(new FILE_LINE(0) cConfigItem_string("diameter_user_session_id_strip_regex", &opt_diameter_user_session_id_strip_regex));
				addConfigItem(new FILE_LINE(0) cConfigItem_string("diameter_callid_strip_regex", &opt_diameter_callid_strip_regex));
