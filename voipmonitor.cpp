...
int opt_update_schema = 0; // --update-schema command line option... creates or updates the schema, and then exits
...
char mysql_user[256] = "voipmonitor";
...
int main(int argc, char *argv[]) {
		...
		switch (c) {
			...
			*/
			...
			case 'X':
				opt_test = 1;
				break;
			case 999: // --update-schema
				opt_update_schema = 1;
				break;
		}
	}
	if (opt_update_schema) {
		// force no forking
		opt_fork = 0;
		// use root user for the database connection
		strncpy(mysql_user, "root", sizeof(mysql_user));

	} else if (configfile[0]) {
		// not updating schema, so allow config file to load if defined
		load_config(configfile);
	}
	...
	if ((fname == NULL) && (ifname[0] == '\0') && opt_scanpcapdir[0] == '\0'){
			...
			*/
		printf("\nvoipmonitor version %s-tghs-customized\n"
