...
void cConfig::addConfigItems() {
	...
	// check
	//  - php_path vs keycheck (dupilicty ?)
	...
	// INTERFACE - READ PACKETS
	group("interface - read packets");
	...
	group("cisco skinny");
			...
			addConfigItem((new cConfigItem_integer("skinny_ignore_rtpip", &opt_skinny_ignore_rtpip))
				->setIp());
	// STORING PACKETS INTO PCAP FILES, GRAPH, AUDIO
	group("storing packets into pcap files, graph, audio");
	...
	// OTHER - PLANETEL
	group("other - ipaccount extended");
				expert();
