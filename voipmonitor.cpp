...
int opt_remotepartyid_calling = 0;	//Rewrite caller? If sip invite contain header Remote-Party-ID, caller num/name is overwritten by its values.
...
int eval_config(string inistr) {
	...
	if((value = ini.GetValue("general", "remotepartyid_calling", NULL))) {
		opt_remotepartyid_calling = yesno(value);
	}
