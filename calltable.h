...
#define FLAG_SAVEBFCP			CALL_FLAG(_CALL_BIT_SAVEBFCP)
...
#define CDR_PCAP_DUMP_ERROR_DLT			CDR_FLAG(_CDR_BIT_PCAP_DUMP_ERROR_DLT)
...
struct s_sdp_media_data {
	...
	u_int16_t ptime;
	vmIP sdp_srflx_addr[5];
	u_int8_t sdp_srflx_addr_count;
};
...
public:
	...
	bool seenmessage;
	bool seenmessageok;
	volatile bool seenbye;
	volatile u_int64_t seenbye_time_usec;
	volatile bool seenokbye;
	volatile u_int64_t seenokbye_time_usec;
	volatile bool seenbye_and_ok;
	bool seenbye_and_ok_permanent;
	volatile u_int64_t seenbye_and_ok_time_usec;
	volatile bool seencancel;
	volatile u_int64_t seencancel_time_usec;
	volatile bool seencancel_and_ok;
	volatile u_int64_t seencancel_and_ok_time_usec;
	volatile bool seenauthfailed;
	volatile u_int64_t seenauthfailed_time_usec;
	volatile u_int64_t ignore_rtp_after_response_time_usec;
...
public:
	...
	*/
	 ...
	 *
	*/
	inline node_call_rtp *hashfind_by_ip_port(vmIP addr, vmPort port, bool lock = true, int type_addr_filter = -1) {
		...
		for(node_call_rtp_ip_port *node = calls_hash[h]; node != NULL; node = node->next) {
			if((node->port == port) && (node->addr == addr)) {
				if(type_addr_filter >= 0) {
					for(node_call_rtp *n = node->calls; n != NULL; n = n->next) {
						if(n->type_addr == (u_int8_t)type_addr_filter) {
							rslt = n;
							break;
						}
					}
