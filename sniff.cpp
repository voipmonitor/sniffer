			...
			if(sip_method == INVITE) {
				...
				*/
				...
				if(!call->has_second_merged_leg or (call->has_second_merged_leg and !merged)) {
					//do not set destroy for BYE which belongs to second leg in case of merged legs through sip header 
					call->destroy_call_at = header->ts.tv_sec + 60;
				...
				if(!call->has_second_merged_leg or (call->has_second_merged_leg and !merged)) {
					//do not set destroy for CANCEL which belongs to second leg in case of merged legs through sip header 
