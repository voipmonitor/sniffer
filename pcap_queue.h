...
struct sHeaderPacketPQout {
	...
	void alloc_and_copy_blockstore() {
		if(block_store) {
			...
			u_char *alloc_packet = new FILE_LINE(16001) u_char[header->get_caplen()];
...
struct pcapProcessData {
	pcapProcessData() {
		...
		if(opt_dup_check) {
			this->prevmd5s = new FILE_LINE(16001) unsigned char[65536 * MD5_DIGEST_LENGTH]; // 1M
...
struct sHeaderPacket {
	...
	inline void alloc(size_t snaplen) {
		header = new FILE_LINE(16001) pcap_pkthdr;
		packet = new FILE_LINE(16001) u_char[snaplen];
	}
	inline void free() {
		if(header) {
			delete header;
...
public:
	PcapQueue_HeaderPacketStack(unsigned int size) {
		...
		hpp_get_size = 0;
		stack = new FILE_LINE(16001) rqueue_quick<sHeaderPacketPool>(size, 0, 0, NULL, false, __FILE__, __LINE__);
	}
...
public:
	...
	struct hpi_batch {
		hpi_batch(uint32_t max_count) {
			...
			this->hpis = new FILE_LINE(16001) hpi[max_count];
...
public:
	...
	struct sBatchHP {
		sBatchHP(unsigned max_count) {
			count = 0;
			used = 0;
			batch = new FILE_LINE(16001) sHeaderPacketPQout[max_count];
			this->max_count = max_count;
		}
		~sBatchHP() {
			delete [] batch;
