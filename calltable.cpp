	...
	for(int i = 0; i < c_branch->ipport_n; i++) {
		if(c_branch->ip_port[i].addr == addr && c_branch->ip_port[i].port == port) {
			...
			if(sdp_flags != c_branch->ip_port[i].sdp_flags) {
				if(c_branch->ip_port[i].sdp_flags.is_mt_image()) {
			...
			if(c_branch->ip_port[sessidIndex].sip_src_addr == sip_src_addr &&
			   (c_branch->ip_port[sessidIndex].addr != addr ||
				...
				if(opt_rtcp) {
					...
					if(!sdp_flags.rtcp_mux && !sdp_flags.is_mt_application()) {
...
bool Call::read_rtp(CallBranch *c_branch, packet_s_process_0 *packetS, int iscaller, bool find_by_dest, bool stream_in_multiple_calls, s_sdp_flags_base sdp_flags, char enable_save_packet, char *ifname) {
	...
	if(opt_enable_ssl && opt_srtp_rtp_dtls_decrypt && packetS->isDtlsHandshake()) {
		...
		return(true);
	} else if(sdp_flags.is_mt_mrcp() && packetS->pflags.is_mrcp()) {
		if(enable_save_packet) {
			save_packet(this, packetS, _t_packet_mrcp, 0, 0, __FILE__, __LINE__);
		}
		return(true);
	} else if(sdp_flags.is_mt_bfcp() && packetS->pflags.is_bfcp()) {
		if(enable_save_packet) {
	...
	*disable_save = false;
	...
	if(packetS->isRtp()) {
		...
		if(RTP::getVersion(data) == 2) {
			...
			curpayload = sdp_flags.is_mt_video() ? PAYLOAD_VIDEO : RTP::getPayload(data);
		} else {
			return(false);
		}
...
Call::_save_rtp(packet_s_process_0 *packetS, s_sdp_flags_base sdp_flags, char enable_save_packet, bool record_dtmf, u_int8_t forceVirtualUdp) {
	...
	if(opt_fax_create_udptl_streams) {
		if(sdp_flags.is_mt_image() && packetS->okDataLenForUdptl()) {
		...
		if(sdp_flags.is_mt_image() && packetS->isUdptlOkDataLen()) {
			UDPTLFixedHeader *udptl = (UDPTLFixedHeader*)packetS->data_();
			if(udptl->data_field) {
				unsigned seq = htons(udptl->sequence);
	...
	if(enable_save_packet) {
		if((this->silencerecording || (this->flags & (sdp_flags.is_mt_video() ? FLAG_SAVERTP_VIDEO_HEADER : FLAG_SAVERTPHEADER))) && 
		...
		} else if((this->flags & (sdp_flags.is_mt_video() ? FLAG_SAVERTP_VIDEO : FLAG_SAVERTP)) || 
			  (this->is_fax() && this->is_fax_packet(packetS)) || 
			  record_dtmf) {
...
Call::saveToDb(bool enableBatchIfPossible) {
	...
	*/
	...
	sRtcpXrStreams rtcp_xr_streams;
	...
	if(rtp_size() > 0) {
		...
		#endif
	} else if(rtcp_xr_streams_ab[0] || rtcp_xr_streams_ab[1]) {
		for(int i = 0; i < 2; i++) {
			...
			for(int j = 0; j < 2; j++) {
				if(!streams[j]) continue;
				if(streams[j]->type == _rtcp_data_type_publish || streams[j]->type == _rtcp_data_type_rtcp_xr) {
					if(existsColumns.cdr_mos_xr and streams[j]->mos_lq_counter > 0) {
						if(streams[j]->mos_lq_min > 0) {
							cdr.add(LIMIT_TINYINT_UNSIGNED(streams[j]->mos_lq_min), c+"_mos_xr_min_mult10");
						}
						if(streams[j]->mos_lq_avg > 0) {
							cdr.add(LIMIT_TINYINT_UNSIGNED(streams[j]->mos_lq_avg), c+"_mos_xr_mult10");
						}
					}
				} else if(streams[j]->type == _rtcp_data_type_rtcp_sr_rr) {
					if(streams[j]->loss_counter > 0) {
						if(existsColumns.cdr_ab_rtcp_loss_is_smallint_type[i]) {
							cdr.add(LIMIT_SMALLINT_SIGNED_TO_UNSIGNED(streams[j]->loss), c+"_rtcp_loss");
						} else if(existsColumns.cdr_ab_rtcp_loss_is_signed_type[i]) {
							cdr.add(LIMIT_MEDIUMINT_SIGNED(streams[j]->loss), c+"_rtcp_loss");
						} else {
							cdr.add(LIMIT_MEDIUMINT_SIGNED_TO_UNSIGNED(streams[j]->loss), c+"_rtcp_loss");
						}
					}
					if(streams[j]->fr_lost_counter > 0) {
						cdr.add(LIMIT_SMALLINT_UNSIGNED(streams[j]->fr_lost_max), c+"_rtcp_maxfr");
						cdr.add(LIMIT_SMALLINT_UNSIGNED((int)round(streams[j]->fr_lost_avg * 10)), c+"_rtcp_avgfr_mult10");
					}
					if(streams[j]->jitter_counter > 0 && streams[j]->ticks_bycodec > 0) {
						int rtcp_maxjitter = (int)round((double)streams[j]->jitter_max / streams[j]->ticks_bycodec);
						int rtcp_avgjitter_mult10 = (int)round(streams[j]->jitter_avg / streams[j]->ticks_bycodec * 10);
						if(rtcp_maxjitter * 10 < rtcp_avgjitter_mult10) {
							++rtcp_maxjitter;
						}
						cdr.add(LIMIT_SMALLINT_UNSIGNED(rtcp_maxjitter), c+"_rtcp_maxjitter");
						cdr.add(LIMIT_SMALLINT_UNSIGNED(rtcp_avgjitter_mult10), c+"_rtcp_avgjitter_mult10");
					}
				}
			}
		}
		if(!rtcp_xr_streams_ab[0] || !rtcp_xr_streams_ab[1]) {
			for(int i = 0; i < 2; i++) {
				if(!rtcp_xr_streams_ab[i]) continue;
				string c = i == 0 ? "b" : "a";
...
void Call::sRtcpXrStreamData::add_loss(int32_t loss) {
	loss_counter++;
	if(loss > this->loss) {
		this->loss = loss;
	}
...
void Call::sRtcpXrStreamData::add_jitter(u_int32_t jitter) {
	jitter_counter++;
	if(jitter > jitter_max) {
		jitter_max = jitter;
	}
	jitter_avg = (jitter_avg * (jitter_counter - 1) + jitter) / jitter_counter;
}
...
void Call::sRtcpXrStreams::findAB(sRtcpXrStreamData *ab[]) {
	...
	if(data[indexes[0]]->iscaller != data[indexes[1]]->iscaller) {
		if(data[indexes[0]]->iscaller) {
			ab[0] = data[indexes[0]];
			ab[1] = data[indexes[1]];
		} else {
			ab[0] = data[indexes[1]];
...
Call::sRtcpXrStreamData *Call::sRtcpXrStreams::getOtherType(sRtcpXrStreamData *stream) {
	sRtcpStreamIndex index(stream->ip_port_src_orig, stream->ip_port_dst_orig, stream->ssrc);
	map<sRtcpStreamIndex, sRtcpXrStreamDataByType>::iterator iter_index = by_type.find(index);
	if(iter_index == by_type.end()) {
		return(NULL);
	}
	for(map<eRtcpDataItemType, sRtcpXrStreamData>::iterator iter_type = iter_index->second.data.begin(); iter_type != iter_index->second.data.end(); iter_type++) {
		if((iter_type->second.type == _rtcp_data_type_rtcp_sr_rr ? 1 : 0) != (stream->type == _rtcp_data_type_rtcp_sr_rr ? 1 : 0)) {
...
void Call::prepareRtcpXrData(sRtcpXrStreams *streams, bool checkOK) {
	...
	for(list<sRtcpDataItem>::iterator iter_rtcp = this->rtcpData.begin(); iter_rtcp != this->rtcpData.end(); iter_rtcp++) {
		if(iter_rtcp->branch_id == c_branch->branch_id) {
			...
			if(iter_rtcp->loss_set) {
				data->add_loss(iter_rtcp->loss);
			}
			if(iter_rtcp->jitter_set) {
				data->add_jitter(iter_rtcp->jitter);
			}
			++data->counter;
		}
	}
	for(map<sRtcpStreamIndex, sRtcpXrStreamDataByType>::iterator iter_index = streams->by_type.begin(); iter_index != streams->by_type.end(); iter_index++) {
		for(map<eRtcpDataItemType, sRtcpXrStreamData>::iterator iter_type = iter_index->second.data.begin(); iter_type != iter_index->second.data.end(); iter_type++) {
			bool is_rtcp = iter_type->first == _rtcp_data_type_rtcp_xr || iter_type->first == _rtcp_data_type_rtcp_sr_rr;
			bool rtcp_mux;
			...
			if(index_ip_port >= 0) {
				...
				index_ip_port = get_index_by_ip_port(c_branch, iter_index->first.ip_port_src.ip, iter_index->first.ip_port_src.port, false, is_rtcp, &rtcp_mux);
				if(index_ip_port >= 0) {
					iter_type->second.ok_by_sdp = true;
			...
			iter_type->second.ip_port_src_orig = iter_index->first.ip_port_src;
			iter_type->second.ip_port_dst_orig = iter_index->first.ip_port_dst;
			...
			if(is_rtcp) {
				if(!rtcp_mux) {
					...
					iter_type->second.rtcp_mux = true;
				}
			}
			if(index_ip_port >= 0) {
				if(opt_rtpmap_by_callerd) {
					if(iscaller_is_set(iter_type->second.iscaller)) {
						iter_type->second.ticks_bycodec = getTicksByCodecIfEq(c_branch, iter_type->second.iscaller);
					}
				} else {
					iter_type->second.ticks_bycodec = getTicksByCodecIfEq(c_branch, index_ip_port);
				}
			}
		}
	}
...
void Calltable::hashAdd(vmIP addr, vmPort port, u_int64_t time_us, CallBranch *c_branch, int iscaller, int is_rtcp, s_sdp_flags sdp_flags, u_int8_t type_addr) {
	...
	if(sverb.hash_rtp
	   ...
	   || c_branch->call->debug_rtp == true
		...
		cout << "hashAdd: "
		     ...
		     << (type_addr == ip_port_call_info::_ta_sdp_candidate ? "sdp_candidate " : "")
		     << endl;
	}
...
inline node_call_rtp *insert_node_call(node_call_rtp *&begin, CallBranch *c_branch, int iscaller, int is_rtcp, s_sdp_flags *sdp_flags, u_int8_t type_addr = 0) {
	...
	node_new->type_addr = type_addr;
	begin = node_new;
	return(node_new);
}

inline void replace_node_call(node_call_rtp *node, CallBranch *c_branch, int iscaller, int is_rtcp, s_sdp_flags *sdp_flags, u_int8_t type_addr = 0) {
	__SYNC_INC(c_branch->rtp_ip_port_counter);
...
void Calltable::_hashAdd(vmIP addr, vmPort port, long int time_s, CallBranch *c_branch, int iscaller, int is_rtcp, s_sdp_flags sdp_flags, u_int8_t type_addr, bool useLock) {
		...
		if ((node->port == port) && (node->addr == addr)) {
			...
			if(!found) {
				if(opt_sdp_multiplication == 0 && count == 1 && node->calls && node->calls->c_branch) {
					replace_node_call(node->calls, c_branch, iscaller, is_rtcp, &sdp_flags, type_addr);
				} else {
					...
					insert_node_call(node->calls, c_branch, iscaller, is_rtcp, &sdp_flags, type_addr);
				}
			}
			if (useLock) unlock_calls_hash();
			return;
...
void Calltable::_hashAddExt(vmIP addr, vmPort port, long int time_s, CallBranch *c_branch, int iscaller, int is_rtcp, s_sdp_flags sdp_flags, u_int8_t type_addr, bool useLock) {
	_hashAdd(addr, port, time_s, c_branch, iscaller, is_rtcp, sdp_flags, type_addr, useLock);
}
	...
	if(hash_modify_queue_begin_ms) {
		if(getTimeMS_rdtsc() >= hash_modify_queue_begin_ms + hash_modify_queue_length_ms) {
			...
			for(list<sHashModifyData>::iterator iter = hash_modify_queue.begin(); iter != hash_modify_queue.end(); iter++) {
				switch(iter->oper) {
				case hmo_add:
					_hashAdd(iter->addr, iter->port, iter->time_s, iter->c_branch, iter->iscaller, iter->is_rtcp, iter->sdp_flags, iter->type_addr, false);
					break;
				case hmo_remove:
...
Calltable::add(int call_type, char *call_id, unsigned long call_id_len, vector<string> *call_id_alternative,
	       ...
	       pcap_t *handle, int dlt, int sensorId, int8_t ci, map<string, Call*> *map_calls) {
	...
	if(cc_data->closeCallsCount) {
		...
		if(!opt_call_id_alternative[0]) {
			...
			for(list<Call*>::iterator iter = calltable->calls_list.begin(); iter != calltable->calls_list.end(); ) {
				if((*iter)->isClosed()) {
					calltable->calls_list.erase(iter++);
				} else {
					iter++;
				}
			}
		...
		if(calls_by_stream_callid_listMAP.size()) {
			for(map<sStreamIds2, Call*>::iterator iter = calls_by_stream_callid_listMAP.begin(); iter != calls_by_stream_callid_listMAP.end(); ) {
				if(iter->second->isClosed()) {
					calls_by_stream_callid_listMAP.erase(iter++);
				} else {
					iter++;
				}
			}
...
string printCallFlags(unsigned long int flags) {
	...
	if(flags & FLAG_SAVEMRCP)			outStr << "savemrcp ";
	if(flags & FLAG_SAVEBFCP)			outStr << "savebfcp ";
