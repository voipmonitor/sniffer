...
void HttpDataCache_relation::addResponse(u_int64_t timestamp, const char *http, const char *body) {
	...
	this->responses[timestamp] = new FILE_LINE(12001) HttpDataCache_data(
						NULL, NULL,
						http, http_md5.c_str(),
						body, body_md5.c_str(),
						NULL, NULL, NULL);
...
void HttpDataCache_link::addRequest(u_int64_t timestamp,
	...
	HttpDataCache_relation *new_relation = new FILE_LINE(12002) HttpDataCache_relation();
	new_relation->request = new FILE_LINE(12003) HttpDataCache_data(
					url, url_md5.c_str(),
					http, http_md5.c_str(),
					body, body_md5.c_str(),
...
void HttpPacketsDumper::dumpDataItem(eReqResp reqResp, string header, string body,
	...
	while(dataOffset < data.length()) {
		...
		u_char *packet = new FILE_LINE(12004) u_char[packetLen];
