...
RRD_FILE="/var/spool/voipmonitor/rrd/2db-heap.rrd"
DATA_FILE="/tmp/rrd_buffer_data.txt"
START_TIME=$(date -d '00:00 today' +%s)
START_TIME="now-2days"
END_TIME="now"

# Získání dat z RRD a filtrování řádků, kde druhý sloupec (buffer) není '-nan'
...
set title "Buffer values from RRD"
...
plot "$DATA_FILE" using 1:2 with lines title "Buffer"
EOF
