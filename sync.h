...
#define __SYNC_TEST_LOCK(vint) (__sync_lock_test_and_set(&(vint), 1))

#if defined __ATOMIC_RELEASE
#define __SYNC_NULL(vint) __atomic_store_n(&(vint), 0, __ATOMIC_RELEASE)
#define __SYNC_SET(vint) __atomic_store_n(&(vint), 1, __ATOMIC_RELEASE)
#define __SYNC_SET_TO(vint, to) __atomic_store_n(&(vint), to, __ATOMIC_RELEASE)
#else
#define __SYNC_NULL(vint) __sync_lock_release(&(vint)) 
#define __SYNC_SET(vint) ((void)__sync_lock_test_and_set(&(vint), 1))
#define __SYNC_SET_TO(vint, to) ((void)__sync_lock_test_and_set(&(vint), to))
#endif
