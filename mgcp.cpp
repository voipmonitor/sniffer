...
void *handle_mgcp(packet_s_process *packetS) {
	...
	if(call) {
		...
		if(sdp) {
			...
			#else
			call->check_is_caller_called(&call->first_branch, NULL, MGCP, 0, 
						     packetS->saddr_(), packetS->daddr_(), 
						     packetS->saddr_(true), packetS->daddr_(true), packetS->header_ip_protocol(true),
						     packetS->source_(), packetS->dest_(), 
						     &iscaller, NULL);
			process_sdp(call, &call->first_branch, packetS, iscaller, (char*)(sdp + sdp_separator_length), 0,
				    (char*)call->call_id.c_str());
			#endif
		}
