		...
		if(sverb.memory_stat) {
			if(memory_type1) {
				...
				if(iter == memoryStatType.end()) {
					...
					if(tid) {
						__SYNC_DEC(threadRecursion[tid]);
					}
				} else {
				...
				if(!threadRecursion[tid]) {
					if(threadStackSize[tid]) {
						...
						__SYNC_ADD(memoryStatOther[beginEx->memory_type_other], sizeOfObject);
					} else {
					...
					__SYNC_ADD(memoryStatOtherSum, sizeOfObject);
				}
			} else {
				__SYNC_ADD(memoryStatOtherSum, sizeOfObject);
			}
		...
		if(HEAPSAFE_CMP_BEGIN_MEMORY_CONTROL_BLOCK(beginMemoryBlock->stringInfo)) { 
			...
			if(sverb.memory_stat) {
				if(beginMemoryBlock->memory_type) {
					__SYNC_SUB(memoryStat[beginMemoryBlock->memory_type], beginMemoryBlock->length);
				} else if(MCB_STACK && ((sHeapSafeMemoryControlBlockEx*)beginMemoryBlock)->memory_type_other) {
					__SYNC_SUB(memoryStatOther[((sHeapSafeMemoryControlBlockEx*)beginMemoryBlock)->memory_type_other], beginMemoryBlock->length);
				} else {
					__SYNC_SUB(memoryStatOtherSum, beginMemoryBlock->length);
				}
			}
...
#if HEAPSAFE
inline void *heapsafe_new(size_t sizeOfObject, size_t align, const char *memory_type1, int memory_type2, int alloc_number, bool arrayForm) {
	if(align) {
		if(align < 16) {
			align = 16;
		}
	...
	if(sizeOfObject > 1000000000ull) {
		if(memory_type1) {
			syslog(LOG_WARNING, "too big allocated block - %zd, %s, %i", sizeOfObject, memory_type1, memory_type2);
		} else {
	...
	void *newPointer = HeapSafeCheck ?
			   ...
			   MemoryStatQuick ?
			    ...
			    _heapsafe_alloc_align(align ? align : heapsafe_align_for_size(sizeOfObject), sizeOfObject);
	if(!newPointer) {
		notEnoughFreeMemory = true;
		if(memory_type1) {
			syslog(LOG_ERR, "allocation (operator new%s%s) failed - size %zd, %s, %i", arrayForm ? "[]" : "", align ? " align" : "", sizeOfObject, memory_type1, memory_type2);
		} else {
			syslog(LOG_ERR, "allocation (operator new%s%s) failed - size %zd", arrayForm ? "[]" : "", align ? " align" : "", sizeOfObject);
		}
	}
	return(newPointer);
}
...
void * operator new(size_t sizeOfObject) {
	return(heapsafe_new(sizeOfObject, 0, NULL, 0, 0, false));
}
 
void * operator new[](size_t sizeOfObject) {
	return(heapsafe_new(sizeOfObject, 0, NULL, 0, 0, true));
}

void * operator new(size_t sizeOfObject, const char *memory_type1, int memory_type2, int alloc_number) {
	return(heapsafe_new(sizeOfObject, 0, memory_type1, memory_type2, alloc_number, false));
}
 
void * operator new[](size_t sizeOfObject, const char *memory_type1, int memory_type2, int alloc_number) {
	return(heapsafe_new(sizeOfObject, 0, memory_type1, memory_type2, alloc_number, true));
}
...
void * operator new(size_t sizeOfObject, std::align_val_t align) {
	return(heapsafe_new(sizeOfObject, (size_t)align, NULL, 0, 0, false));
}

void * operator new[](size_t sizeOfObject, std::align_val_t align) {
	return(heapsafe_new(sizeOfObject, (size_t)align, NULL, 0, 0, true));
}

void * operator new(size_t sizeOfObject, std::align_val_t align, const char *memory_type1, int memory_type2, int alloc_number) {
	return(heapsafe_new(sizeOfObject, (size_t)align, memory_type1, memory_type2, alloc_number, false));
}

void * operator new[](size_t sizeOfObject, std::align_val_t align, const char *memory_type1, int memory_type2, int alloc_number) {
	return(heapsafe_new(sizeOfObject, (size_t)align, memory_type1, memory_type2, alloc_number, true));
}
