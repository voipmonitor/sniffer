...
cRegisterFilterItem::cRegisterFilterItem(eTypeRegisterFilterItem filterType) {
	this->filterType = filterType;
}

void cRegisterFilterItem::addValue(const char *value) {
	fValues.push_back(value);
}

bool cRegisterFilterItem::check(RegisterRecord *rec) {
	for(list<string>::iterator iter = fValues.begin(); iter != fValues.end(); iter++) {
		if(this->checkValue(&(*iter), rec)) {
			return(true);
		}
	}
	return(false);
...
bool cRegisterFilterItem::checkValue(string *value, RegisterRecord *rec) {
	switch(filterType) {
	case rfi_digestusername:
		if(rec->fields[rf_digestusername].s &&
		   strcasestr(rec->fields[rf_digestusername].s, value->c_str())) {
			return(true);
		}
		break;
	default:
...
cRegisterFilter::cRegisterFilter(char *filter) {
	...
	for(unsigned int i = 0; i < jsonData.getLocalCount(); i++) {
		...
		cRegisterFilterItem::eTypeRegisterFilterItem filterType = cRegisterFilterItem::rfi_na;
		if(filterTypeName == "digestusername") {
			filterType = cRegisterFilterItem::rfi_digestusername;
		}
		if(filterType) {
			map<cRegisterFilterItem::eTypeRegisterFilterItem, cRegisterFilterItem>::iterator iter = fItems.find(filterType);
			if(iter == fItems.end()) {
				fItems[filterType] = cRegisterFilterItem(filterType);
			}
			fItems[filterType].addValue(filterValue.c_str());
		}
	}
}

bool cRegisterFilter::check(struct RegisterRecord *rec) {
	map<cRegisterFilterItem::eTypeRegisterFilterItem, cRegisterFilterItem>::iterator iter;
	for(iter = fItems.begin(); iter !=fItems.end(); iter++) {
		if(!iter->second.check(rec)) {
			return(false);
		}
	}
	return(true);
